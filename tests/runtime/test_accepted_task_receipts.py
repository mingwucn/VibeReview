...
def test_fallback_accepted_receipt_reuses_complete_ordered_engine_plan(tmp_path: Path):
    runtime = _create_runtime(tmp_path)
    invocation = GenerateCandidateClaimsInvocation(topic="fallback receipt", existing_theme_ids=[])
    primary = MockEngine(
...
def test_fallback_plan_order_drift_prevents_receipt_reuse(tmp_path: Path):
    runtime = _create_runtime(tmp_path)
    invocation = GenerateCandidateClaimsInvocation(topic="plan drift", existing_theme_ids=[])
    first = runtime.run(
        TaskType.GENERATE_CANDIDATE_CLAIMS,
        invocation,
        engines=[
            MockEngine([MockResponse(execution_error="temporary")], name="primary"),
            MockEngine(
                [MockResponse(proposal=_discovery_proposal("plan").model_dump(mode="json"))],
                name="fallback",
            ),
        ],
    )
    assert first.engine == "fallback"

    # Reordering changes the plan key, so the engine is executed.  Its valid
    # proposal is intentionally distinct to permit a second canonical commit.
    reordered = MockEngine(
        [MockResponse(proposal=_discovery_proposal("reordered").model_dump(mode="json"))],
        name="fallback",
        version="2",
    )
    second = runtime.run(
        TaskType.GENERATE_CANDIDATE_CLAIMS,
        invocation,
        engines=[reordered, MockEngine([], name="primary")],
    )
    assert second.receipt_reused is False
    assert reordered.calls == 1
    assert runtime.store.current_generation() == 2


def test_technical_attempt_count_drift_prevents_receipt_reuse(tmp_path: Path):
    project = tmp_path / "project"
    first_runtime = ProjectRuntime.create(
        project,
        project_name="receipt-tests",
        config=RuntimeConfig(technical_attempts_per_engine=2),
    )
    invocation = GenerateCandidateClaimsInvocation(
        topic="attempt-plan drift", existing_theme_ids=[]
    )
    first_engine = MockEngine(
        [
            MockResponse(execution_error="retryable provider failure"),
            MockResponse(
                proposal=_discovery_proposal("retried").model_dump(mode="json")
            ),
        ],
        name="retrying-engine",
    )
    ...
    second_runtime = ProjectRuntime.create(
        project,
        project_name="receipt-tests",
        config=RuntimeConfig(technical_attempts_per_engine=1),
    )
    second_engine = MockEngine(
        [
            MockResponse(
                proposal=_discovery_proposal("single").model_dump(mode="json")
            )
        ],
        name="retrying-engine",
    )
...
def test_unrelated_generation_leaves_receipt_reusable(tmp_path: Path):
    ...
    def add_unrelated(snapshot, registry):
        identifiers, registry = registry.allocate(IdKind.THEME, 98)
        theme = ThemeRecord(
            theme_id=identifiers[-1],
