...
def test_storage_load_rejects_group_readable_or_symlinked_authority(tmp_path: Path):
    probe, report, fingerprint, qualification = _valid_gate_inputs()
    root = tmp_path / "store"
    path = store_qualification(qualification, report, root=root)
    assert path.stat().st_mode & 0o777 == 0o600
    assert root.stat().st_mode & 0o777 == 0o700

    path.chmod(0o640)
    assert load_qualification_for_fingerprint(fingerprint, root=root) is None
    path.chmod(0o600)
    root.chmod(0o750)
    assert load_qualification_for_fingerprint(fingerprint, root=root) is None
    root.chmod(0o700)
    ...
    assert load_qualification_for_fingerprint(fingerprint, root=alias) is None
