...
from vibereview.runtime import execution as execution_module
...
def test_unverified_session_cleanup_overrides_success_and_prevents_commit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    real_rmtree = execution_module.shutil.rmtree
    leaked: list[Path] = []

    def no_op(path, *args, **kwargs):
        leaked.append(Path(path))

    monkeypatch.setattr(execution_module.shutil, "rmtree", no_op)
    runtime, result = _run(tmp_path, [_engine("cleanup-injected", ("valid",))])
    ...
    assert any(
        failure.code == "execution_session_cleanup_failed"
        for failure in result.attempt_records[0].detected_failures
    )
    monkeypatch.setattr(execution_module.shutil, "rmtree", real_rmtree)
    for path in leaked:
        if path.name.startswith("vibereview-exec-") and path.exists():
            real_rmtree(path)
