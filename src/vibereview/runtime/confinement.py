...
def compute_confinement_code_fingerprint() -> str:
    ...
    # Qualification authority is deliberately invalidated by drift in any
    # runtime module.  This conservative closure covers factories, receipt
    # reuse, parsing/validation, credential locks, and acceptance precedence,
    # including future runtime modules without relying on a hand-maintained list.
    files = sorted(here.glob("*.py"))
    return code_fingerprint(files, contract_version="1.8")
