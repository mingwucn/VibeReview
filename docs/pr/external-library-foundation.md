...
fixture, structural validator, or model-generated audit in this change is a
scientific or human sign-off.

## Intended scope

- Bounded compilation of every task-manifest dependency and resource.
- Explicit offline versus qualified live engine construction.
- Credential-safe DeepSeek REST transport and bounded inert protocol adapters
  for Kimi, Agy, and Codex, whose live factories remain explicitly unavailable
  pending machine-local qualification.
- A generic pinned-Git library foundation using immutable object reads and
  synthetic fixtures, without a bundled external checkout or operator config.
- Content-addressed, writer-locked, idempotent corpus import and retrieval that
  returns candidates rather than manufacturing canonical evidence.
...
- Live construction fails closed if qualification, credential isolation,
